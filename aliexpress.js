  ...
  return new Promise((resolve, reject) => {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $(details_parsed)
          .find("table.fund-table#tp-buyer-order-table>tbody>tr.fund-bd")
          .each(function () {
            if ($(this).find('td.pay-c3:contains("redit")').length != 0) {
              total_credit_card = $(this)
                .find("td.pay-c1")
                .text()
                .trim()
                .replace(/CA C\$ /gm, "");
        ...
        console.log(
          "loaded: " +
            JSON.stringify({
              details_price: $(details_parsed)
                .find("tbody>tr>td.product-price")
                .text()
                .trim(),
              details_shipping: $(details_parsed)
                .find("tbody>tr>td.shipping-price")
                .text()
                .trim(),
              details_adjust: $(details_parsed)
                .find("tbody>tr>td.change-price")
                .text()
                .trim(),
              details_discount: $(details_parsed)
                .find("tbody>tr>td.discount-price")
                .text()
                .trim(),
              details_total: $(details_parsed)
                .find("tbody>tr>td.order-price")
                .text()
                .trim(),
              details_payments: $(details_parsed)
                .find("table.fund-table>tbody>tr.fund-bd>td.pay-c1")
                .text()
                .trim(),
        ...
        .each((i, e) => {
          let product = {
            ...
            product_price: $(e)
              ...
              .text()
              .trim(), // remove parcer for different currency
            product_quantity: $(e)
              .find(".product-amount span:eq(1)")
              .text()
              .trim()
              .slice(1),
            product_num: ++inum,
    ...
    return Promise.all(detail_tasks).then((result) => {
      orders.forEach((order) => {
        ...
        order.products.forEach((product) => {
          ...
          var total_order_value =
            cleanPrice(order_detail.details_price) +
            cleanPrice(order_detail.details_shipping);
          var total_savings =
            cleanDiscount(order_detail.details_discount) +
            parseFloat(order_detail.total_coupons);
          var prorated_shipping = round_money(
            (cleanPrice(order_detail.details_shipping) /
              cleanPrice(order_detail.details_price)) *
              cleanPrice(product.product_price)
          );

          var prorated_savings =
            (total_savings /
              (cleanPrice(order_detail.details_price) +
                cleanPrice(order_detail.details_shipping))) *
            (cleanPrice(product.product_price) + prorated_shipping);
          ...
          s +=
            '"' +
            clean(
              round_money(cleanPrice(product.product_price) - prorated_savings)
            ) +
            '"\t';
